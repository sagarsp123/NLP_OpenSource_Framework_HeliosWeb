...
def analyze_sentiment_endpoint():
    import Server.models.nltk as nlp
    if(nltkModel is None):
        nltkModel = nlp.SentimentAnalyzer()

    return runModel(nltkModel, request.get_json())


bertModel = None
@app.route('/nlp_nltk_sentiment', methods=['POST'])
def analyze_sentiment_endpoint():
    import Server.models.bertModel
    if(bertModel is None):
        bertModel = Server.models.bertModel.BertSentimentAnalyzer()

    return runModel(bertModel, request.get_json())
    ...
    try:
        ...
        for key, sentence in json_data.items():
            print(key, sentence)
            try:
                scores = nltkModel.analize(sentence)
            except Exception as e:
                print(e)
                scores = {"error": str(e)}
